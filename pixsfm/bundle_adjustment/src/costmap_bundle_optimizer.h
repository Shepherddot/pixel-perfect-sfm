...
int CostMapBundleOptimizer::AddResiduals(const colmap::image_t image_id,
                                         ...
                                         ceres::LossFunction* loss_function,
                                         FeatureView<dtype>& feature_view) {
  const bool constant_pose = !options_.refine_extrinsics || setup_.HasConstantCamPose(image_id);
  ...
  if (constant_pose) {
    ceres::CostFunction* cost_function =
        CreateFeatureReferenceConstantPoseCostFunctor<CHANNELS, 1, -1>(camera.model_id,
                                                                       qvec_data,
                                                                       tvec_data,
    ...
    ceres::CostFunction* cost_function = CreateFeatureReferenceCostFunctor<CHANNELS, 1, -1>(camera.model_id,
                                                                                            feature_view.GetFeaturePatch(image_id, point2D_idx),
                                                                                            nullptr,  // Just minimize
                                                                                            nullptr,
                                                                                            interpolation_config_);
