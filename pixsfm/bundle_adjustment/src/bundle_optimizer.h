...
void BundleOptimizer<Derived>::AddImageToProblem(
  ...
  for (colmap::point2D_t point2D_idx = 0; point2D_idx < image.NumPoints2D();
    ...
    if (static_cast<int>(point3D.track.Length()) < options_.min_track_length) {
      continue;
    }
...
void BundleOptimizer<Derived>::AddPointToProblem(
  ...
  for (const auto& track_el : point3D.track.Elements()) {
    ...
    if (camera_num_residuals_[image.CameraId()] == 0) {
      setup_.SetConstantCamIntrinsics(image.CameraId());
    }
    int num_blocks = static_cast<Derived*>(this)->template AddResiduals<Ns...>(
    ...
    if (element.second > 0) {
      ...
      if (!constant_pose) {
        ...
        if (setup_.HasConstantCamPositions(image_id)) {
          const std::vector<int>& constant_tvec_idxs = setup_.ConstantCamPositions(image_id);
          colmap::SetSubsetManifold(3, constant_tvec_idxs, problem_.get(), tvec_data);
        }
      } else {
...
template <typename Derived>
void BundleOptimizer<Derived>::ParameterizeCameras(colmap::Reconstruction* reconstruction) {
  ...
  for (const auto& element : camera_num_residuals_) {
    ...
    if (constant_camera || setup_.HasConstantCamIntrinsics(camera_id)) {
      ...
      if (const_camera_params.size() > 0) {
        colmap::SetSubsetManifold(static_cast<int>(camera.params.size()), const_camera_params, problem_.get(), camera.params.data());
      }
    }
  }
}
